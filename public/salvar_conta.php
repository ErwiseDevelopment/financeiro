...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // 1. BUSCAR FECHAMENTO E VENCIMENTO
        $dia_fechamento = 30; 
        $dia_vencimento = 30; // Padrão seguro
        ...
        for ($i = 1; $i <= $limite_repeticao; $i++) {
            ...
            // --- 2. LÓGICA DE FATURA FINAL (MESMA DO JS) ---
            ...
            if ($cartoid) {
                ...
                // B. Regra do Pagamento (O cartão vence no próximo mês?)
                // Se o Vencimento for MENOR que o Fechamento, o pagamento é no mês seguinte ao ciclo
                if ($dia_vencimento < $dia_fechamento) {
                    $meses_add++;
                }

                if($meses_add > 0) {
                    // Adiciona os meses calculados
            ...
            $stmt->execute([
                ...
                $contatipo, $contafixa, $cartoid, $i, $parcelas_total
            ]);
        }
        ...
        if (isset($_POST['manter_dados']) && $_POST['manter_dados'] == '1') {
            $params = http_build_query([
                'msg'  => 'sucesso', 'tipo' => $contatipo, 'cat'  => $categoriaid,
                'car'  => $cartoid ?? '', 'venc' => $contavencimento, 'fixa' => $contafixa
            ]);
